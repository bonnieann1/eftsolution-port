...
def write_redirects() -> None:
    ...
    for frm, to in REDIRECTS.items():
        ...
        # It happens for every entry that only adds the trailing slash — /clients
        # -> /clients/ is right, but the generated /clients/ -> /clients/ is not,
        # and because these rules are forced (301!) the loop beats the real page.
        for src in (frm, frm + "/"):
            if src == to:
                continue
